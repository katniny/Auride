...
export async function loveNote(id) {
    // check for identifier & id type
    if (!id)
        throw new Error("Attempted to love a note without an ID. Please get a note ID!");
    ...
    if (!res.ok)
        throw new Error(`Failed to love note: ${res.status}`);
