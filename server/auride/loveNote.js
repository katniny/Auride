...
router.post("/api/auride/loveNote", async (req, res) => {
    ...
    try {
        ...
        const crementRef = db.ref(`/notes/${noteId}/likes`);
        if (whoLikedKeys.includes(userUidFromRequest)) {
            // unlove
            await db.ref(`/notes/${noteId}/whoLiked/${userUidFromRequest}`).remove();
        ...
        // else, follow
        const loveNote = await db.ref(`/notes/${noteId}/whoLiked/${userUidFromRequest}`).update({
