...
export async function setEmail(email) {
    ...
    // finally, get data
    const res = await apiFetch(`${import.meta.env.VITE_BACKEND_URL}/api/auride/changeEmail`, {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
        ...
        throw new Error(data.error || `Failed to set email with status code ${res.status}.`);
    }
